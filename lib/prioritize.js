 ...
 * [['Users', '0'], ['Items', '1', 'bar']]
 */
function getNeeds(table, entry, extraDependencies) {
  const extra = extraDependencies ? extraDependencies(table, entry) || [] : [];

  if(_.isString(entry)) {
    return extra.concat(getNeedsString(entry));
  } else {
    return extra.concat(getNeedsObject(entry));
  }
}
...
function getNeedsObject(obj) {
  ...
  return values.filter(function(property) {
    return _.isString(property) && property.indexOf(':') > -1 && !/^\d$/.test(property[0]) && property.indexOf('::') === -1;
 ...
 */
function satisfied(table, entry, availablePreReqs, extraDependencies) {
  var needs = getNeeds(table, entry, extraDependencies);
  var missingReqs = [];

  return [_.every(needs, function(need) {
    var preReq = availablePreReqs[need[0]];
    if (preReq &&  _.includes(preReq, need[1])) {
      return true;
    } else {
      missingReqs.push(need);
    }
  }), missingReqs];
}
 ...
 */
module.exports = function prioritize(config, options) {
  config = _.clone(config);
  var extraDependencies = options ? options.extraDependencies : null;
  var prioritized = [];
  ...
  while (Object.keys(config).length) {
    ...
    _.forIn(config, function(entries, table) {
      ...
      entries.forEach(function(entry, index) {
        var ret = satisfied(table, entry, availablePreReqs, extraDependencies);
        ...
        if (ret[0]) {
          ...
          missingReqs = missingReqs.concat(ret[1]);
        }
      });
    ...
    if (_.isEmpty(levelEntries)) {
      return new Error("Non-existant dependency. Unresolved dependencies: " + require("util").format(_.clone(missingReqs))); 
    }
