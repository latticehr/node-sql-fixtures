...
module.exports = function resolveDependencies(availablePreReqs, toBeResolved, options) {
  toBeResolved = _.clone(toBeResolved);
  var extraResolver = options ? options.extraResolver : null;
  ...
  _.forIn(toBeResolved, function(entries, table) {
    toBeResolved[table] = entries.map(function(entry) {
      if(_.isString(entry)) {
        ...
        var extraResolve = extraResolver ? extraResolver(table, entry, function (value) { return resolve(value, availablePreReqs); }) : null;
        if (extraResolve) return entry;
